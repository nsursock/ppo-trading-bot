...
class TradingEnvironment(gym.Env):
    ...
    def open_position(self, symbol_index, type, current_price):
        ...
        # Introduce random slippage
        random_slippage = random.uniform(-0.0001, 0.0001)  # Random slippage between -0.01% and 0.01%
        if type == 'long':
            adjusted_price = current_price * (1 + self.params['slippage'] + self.params['bid_ask_spread'] + random_slippage)
        elif type == 'short':
            adjusted_price = current_price * (1 - self.params['slippage'] - self.params['bid_ask_spread'] - random_slippage)
        
        # Introduce random execution delay
        execution_delay = random.randint(0, 5)  # Random delay between 0 and 5 steps
        self.current_step = min(self.current_step + execution_delay, len(self.data_matrix) - 1)
    ...
    def close_position(self, symbol_index, exit_price, exit_reason="std", exit_time=None):
        ...
        if position['type'] == 'long':
            adjusted_exit_price = exit_price * (1 - self.params['slippage'] - self.params['bid_ask_spread'] - random_slippage)
    ...
    def step(self, action):
        ...
        if not self.live_mode:
            self.current_step = min(self.current_step + 1, len(self.data_matrix) - 1)
