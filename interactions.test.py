...
# 2024-10-02 13:18:14,888 - INFO - Executed trade for NEAR with type open_long and info {'type': 'open_long', 'collateral': 198.49247999999997, 'leverage': 8, 'tp_price': 4.878717928369065, 'sl_price': 4.820641035815467}
    ...
    def test_fetch_open_trades(self):
        try:
            ...
            # Perform assertions
            self.assertIsInstance(open_trades, list, "Open trades should be a list.")
            for trade in open_trades:
                self.assertIsInstance(trade, tuple, "Each trade should be a tuple.")
                self.assertEqual(len(trade), 3, "Each trade tuple should have 3 elements.")
                
        except Exception as error:
            logging.error('Error in test_fetch_open_trades:', exc_info=True)
            self.fail(f"Test failed due to an unexpected error: {error}")
