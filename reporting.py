...
def display_stats(history, market_conditions, environment, save_path='net_profits.png'):
    ...
    # Add new columns for time periods
    if 'open_time' in history_df.columns:
        history_df['open_time_period'] = history_df['open_time'].apply(get_time_period)
    if 'close_time' in history_df.columns:
        history_df['close_time_period'] = history_df['close_time'].apply(get_time_period)
    ...
    for symbol in unique_symbols:
        group = history_df[history_df['symbol'] == symbol]
        stats = {
            'symbol': symbol,
            'num_trades': len(group),
            'win_rate': f"{win_rate(group['return']):.2f}",
            ...
            'num_max': count_exit_reasons(group, 'max'),  # Assuming 'max' is the reason for max price hits
            'avg_leverage': f"{group['leverage'].mean():.2f}" if 'leverage' in group.columns else 'N/A',
            'avg_collateral': f"{group['collateral'].mean():,.2f}" if 'collateral' in group.columns else 'N/A',
            'avg_trade_duration': f"{group['trade_duration'].mean():.2f}" if 'trade_duration' in group.columns else 'N/A',
        }
    ...
    # Unique types
    unique_types = history_df['type'].unique()
    type_stats = []
    
    for trade_type in unique_types:
        group = history_df[history_df['type'] == trade_type]
        stats = {
            'type': trade_type,
            'num_trades': len(group),
            'win_rate': f"{win_rate(group['return']):.2f}",
    ...
    print("\nStats by Type:")
    print(tabulate(type_stats_df, headers="keys", tablefmt="pretty", showindex=False))
    
    # Unique open time periods
    unique_open_time_periods = history_df['open_time_period'].unique()
    open_time_period_stats = []
    
    for time_period in unique_open_time_periods:
        group = history_df[history_df['open_time_period'] == time_period]
        stats = {
            'open_time_period': time_period,
            'num_trades': len(group),
            'win_rate': f"{win_rate(group['return']):.2f}",
            'avg_profit': f"{avg_profit(group['pnl']):,.2f}",
            'avg_loss': f"{avg_loss(group['pnl']):,.2f}",
        ...
        open_time_period_stats.append(stats)
    
    open_time_period_stats_df = pd.DataFrame(open_time_period_stats)
    print("\nStats by Open Time Period:")
    print(tabulate(open_time_period_stats_df, headers="keys", tablefmt="pretty", showindex=False))
    
    # Unique close time periods
    unique_close_time_periods = history_df['close_time_period'].unique()
    close_time_period_stats = []
    
    for time_period in unique_close_time_periods:
        group = history_df[history_df['close_time_period'] == time_period]
        stats = {
            'close_time_period': time_period,
            'num_trades': len(group),
            'win_rate': f"{win_rate(group['return']):.2f}",
            'avg_profit': f"{avg_profit(group['pnl']):,.2f}",
            'avg_loss': f"{avg_loss(group['pnl']):,.2f}",
        ...
        close_time_period_stats.append(stats)
    
    close_time_period_stats_df = pd.DataFrame(close_time_period_stats)
    print("\nStats by Close Time Period:")
    print(tabulate(close_time_period_stats_df, headers="keys", tablefmt="pretty", showindex=False))
    
    # Define leverage and collateral categories
    leverage_bins = [0, environment.params['leverage_min'], environment.params['leverage_max'] / 2, environment.params['leverage_max']]
    collateral_bins = [0, environment.params['collateral_min'], environment.params['collateral_max'] / 2, environment.params['collateral_max']]

    # Add leverage and collateral categories to the DataFrame
    if 'leverage' in history_df.columns:
        history_df['leverage_category'] = pd.cut(history_df['leverage'], bins=leverage_bins, labels=['Low', 'Medium', 'High'])
    if 'collateral' in history_df.columns:
        history_df['collateral_category'] = pd.cut(history_df['collateral'], bins=collateral_bins, labels=['Low', 'Medium', 'High'])

    # Group by leverage category
    if 'leverage_category' in history_df.columns:
        leverage_category_stats = []
        for category in history_df['leverage_category'].unique():
            group = history_df[history_df['leverage_category'] == category]
            stats = {
                ...
                'num_sls': count_exit_reasons(group, 'sl'),
                'num_liqs': count_exit_reasons(group, 'liq'),
                'num_max': count_exit_reasons(group, 'max'),
                'avg_collateral': f"{group['collateral'].mean():,.2f}" if 'collateral' in group.columns else 'N/A',
                'avg_trade_duration': f"{group['trade_duration'].mean():.2f}" if 'trade_duration' in group.columns else 'N/A',
            }
            leverage_category_stats.append(stats)

        leverage_category_stats_df = pd.DataFrame(leverage_category_stats)
        print("\nStats by Leverage Category:")
        print(tabulate(leverage_category_stats_df, headers="keys", tablefmt="pretty", showindex=False))

    # Group by collateral category
    if 'collateral_category' in history_df.columns:
        collateral_category_stats = []
        for category in history_df['collateral_category'].unique():
            group = history_df[history_df['collateral_category'] == category]
            stats = {
            ...
            collateral_category_stats.append(stats)

        collateral_category_stats_df = pd.DataFrame(collateral_category_stats)
        print("\nStats by Collateral Category:")
        print(tabulate(collateral_category_stats_df, headers="keys", tablefmt="pretty", showindex=False))
...
def plot_combined_correlation_heatmap(history, group_by='symbol', save_path='combined_correlation_heatmap.png'):
    ...
    for name, group in history_df.groupby(group_by):
        metrics_data = {
            ...
            'num_tps': count_exit_reasons(group, 'tp'),
            'num_sls': count_exit_reasons(group, 'sl'),
            'num_maxs': count_exit_reasons(group, 'max'),
            'num_liqs': count_exit_reasons(group, 'liq'),
