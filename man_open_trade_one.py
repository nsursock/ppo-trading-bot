...
def decode_revert_reason(error):
    try:
        ...
        logging.debug(f'Error details: {e.args}')  # Add this line to log error details
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Open a single trade for a symbol with specified parameters.')
    ...
    parser.add_argument('-b', '--symbol', type=str, default='CAT', help='Symbol to trade.')
    parser.add_argument('-l', '--leverage', type=int, default=100, help='Leverage to use for the trade.')
    parser.add_argument('-c', '--collateral', type=float, default=99, help='Collateral amount to use for the trade.')
    ...
    # Fetch symbols and latest price
    pairs = fetch_symbols()
    response = requests.get("https://backend-pricing.eu.gains.trade/charts")
    ohlc_data = response.json()

    # Select a single symbol
    symbol = args.symbol
    if symbol not in [pair['symbol'] for pair in pairs]:
        raise ValueError(f"Symbol {symbol} is not available in the pairs list.")
    ...
    # Use the specified leverage and collateral
