...
# from risk_management import handle_risk_management_1s_numpy

def handle_risk_management_1s_loop(symbol_index, position_type, low_prices, high_prices, sl_prices, tp_prices, liq_prices, max_prices, timestamps):
    sl_price = sl_prices[symbol_index]
    tp_price = tp_prices[symbol_index]
    liq_price = liq_prices[symbol_index]
    max_price = max_prices[symbol_index]
    
    num_candles = len(low_prices)
    ...
    for i in range(num_candles):
        ...
        if liq_price is not None:
            if position_type == 1 and low_prices[i] <= liq_price:
                logging.debug(f"Liquidation hit at index {i} for symbol {symbol_index} with price {liq_price} at {timestamps[i]}")
        ...
        if tp_price is not None:
            if position_type == 1 and high_prices[i] >= tp_price:
                logging.debug(f"TP hit at index {i} for symbol {symbol_index} with price {tp_price} at {timestamps[i]}")
        ...
        if sl_price is not None:
            if position_type == 1 and low_prices[i] <= sl_price:
                logging.debug(f"SL hit at index {i} for symbol {symbol_index} with price {sl_price} at {timestamps[i]}")
...
handle_risk_management_optimized = handle_risk_management_1s_loop
...
def send_error_email(subject, body, stack_trace=None):
    ...
    if os.getenv('SEND_EMAILS') == 'true':
        ...
        # Attach the main body and stack trace if available
        full_body = body
        if stack_trace:
            full_body += f"\n\nStack Trace:\n{stack_trace}"
