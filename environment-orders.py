...
class TradingEnvironment(gym.Env):
    def __init__(self, data_matrix, timestamps, mapping, render_mode='human', params=None, reward_function=None, market_data=None, live_mode=False):
        ...
        self.orders = [{} for _ in range(self.num_symbols) ]
    ...
    def handle_orders(self):
        for symbol_index, order in enumerate(self.orders):
            if not order:
                continue  # Skip if there's no order for this symbol

            for i in range(len(self.market_data[symbol_index]['close'])):
                current_price = self.market_data[symbol_index]['close'].iloc[i]
                limit_price = order['limit_price']

                # Check if the order should be fulfilled
                if (order['type'] == 'long' and current_price <= limit_price) or \
                   (order['type'] == 'short' and current_price >= limit_price):
                    
                    # Set the position object
                    self.positions[symbol_index] = {
                        'open_time': self.market_data[symbol_index].index[i],
                        'type': order['type'],
                        ...
                        'volatility_percentage': order['volatility_percentage'],
                        'borrowing_fee': order['borrowing_fee'],
                    }

                    # Log the order fulfillment
                    logging.info(f"Order fulfilled for symbol {self.params['symbols'][symbol_index]} at price {limit_price}")

                    # Remove the fulfilled order to prevent repeated logging
                    self.orders[symbol_index] = {}
                    break  # Exit the loop once the order is fulfilled
    ...
    def open_position(self, symbol_index, type, current_price):
        ...
        # Calculate recent volatility using standard deviation of closing prices from market_data
        recent_closes = self.market_data[symbol_index]['close'].iloc[-300:]  # Use the last 10 closing prices
        recent_volatility = np.std(recent_closes)

        # Determine the percentage based on recent volatility
        volatility_percentage = recent_volatility / current_price

        # Set the limit price based on the type of trade
        if type == 'long':
            limit_price = current_price * (1 - volatility_percentage)
        elif type == 'short':
            limit_price = current_price * (1 + volatility_percentage)
        else:
            limit_price = current_price  # Default to current price if type is not recognized
        ...
        initial_collateral_factor = risk_per_trade
        ...
        position_size = collateral / limit_price

        # Compute SL, TP, and liquidation prices
        sl_price, tp_price, liq_price, max_price = self.compute_prices(symbol_index, type, limit_price, leverage)

        # Adjust limit price for slippage and bid-ask spread
        if type == 'long':
            adjusted_price = limit_price * (1 + self.params['slippage'] + self.params['bid_ask_spread'])
        elif type == 'short':
            adjusted_price = limit_price * (1 - self.params['slippage'] - self.params['bid_ask_spread'])
        ...
        return collateral, leverage, tp_price, sl_price
    ...
    def close_position(self, symbol_index, exit_price, exit_reason="std", exit_time=None):
        ...
        self.history.append({
            ...
            'balance': f"{position['balance']:.2f}",
            'risk_per_trade': f"{position['risk_per_trade'] * 100:.2f}%",
            'volatility_percentage': f"{position['volatility_percentage'] * 100:.2f}%",
        })
    ...
    def handle_risk_management_1s(self, symbol_index):
        ...
        if 'type' in position:
            ...
            # # Assuming timestamps is a numpy array of datetime64
            # timestamps_int = mkt_data.index.values.astype('int64') // 10**9  # Convert to Unix timestamp in seconds
