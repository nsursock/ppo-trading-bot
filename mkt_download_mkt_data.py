...
selected_params['symbols'] = ['ADA', 'BNB', 'EOS', 'ETH', 'IOTA', 'LTC', 'NEO', 'QTUM', 'XLM', 'XRP']
...
selected_params['end_time'] = '2021-12-31'
...
def process_symbol(symbol):
    ...
    start_time = first_day_time.normalize()
    ...
    try:
        for _ in range(selected_params['limit']):  # Loop for the number of limits
            ...
            current_end_time = start_time.replace(hour=23, minute=59, second=59)  # Ensure end of day
            ...
            if os.path.exists(file_path):
                ...
                try:
                    df_klines, start_time2, end_time2 = fetch_binance_klines(
                        symbol, 
                        '1s', 
                        interval_seconds, 
                        current_end_time
                    ...
                    logging.info(f'Start time: {start_time2} | End time: {end_time2}')
            ...
            logging.info(f'Current start time: {start_time} | Current end time: {current_end_time} | Selected end time: {selected_params["end_time"]}')
